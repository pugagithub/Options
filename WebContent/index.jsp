...
<body>
	...
	int cols = tradeList.size();
	int rows = expiryList.length;
	%>



	<table align="center" width="800" cellpadding="0" cellspacing="0"
		bgcolor="yellow" border="1" bordercolor="black">
		<tr>
			<td>Description</td>
			<td>Values</td>
		</tr>
		...
		<tr>
			...
			<td
				<%=(netPremiumPaidOrReceived > 0) ? "bgcolor=red" : "bgcolor=lightgreen"%>><%=(netPremiumPaidOrReceived > 0) ? "Paid" : "Received"%>
				: <%=Math.abs(netPremiumPaidOrReceived)%> [<%=request.getAttribute("netPremiumText")%>]</td>
		</tr>
		...
		<tr>
			...
			<td
				<%=(netPremiumPaidOrReceived > 0) ? "bgcolor=red" : "bgcolor=lightgreen"%>><%=(netPremiumPaidOrReceived > 0) ? "Paid" : "Received"%>
				: <%=Math.abs(netPremiumPaidOrReceived) * lotSize%> [<%=Math.abs(netPremiumPaidOrReceived)%>
				* <%=lotSize%>]</td>
		</tr>
		...
		<tr>
			...
			<td
				<%=(netPremiumPaidOrReceived > 0) ? "bgcolor=red" : "bgcolor=lightgreen"%>><%=(netPremiumPaidOrReceived > 0) ? "Paid" : "Received"%>
				: <%=Math.abs(netPremiumPaidOrReceived) * lotSize * qty%> [<%=Math.abs(netPremiumPaidOrReceived)%>
				* <%=lotSize%> * <%=qty%>]</td>
		</tr>
		...
		<tr>
			<td>No. of Trading Sessions Till Expiry <br> (Incl Both
				Dates)
			</td>
		...
		<tr>
			...
			<td><%=tradeReversalExpense * -1%> [(<%=tradeReversalExpenseText%>)
				* -1]</td>
		</tr>
		...
		<tr>
			...
			<td <%=(mtm > 0) ? "bgcolor=lightgreen" : "bgcolor=red"%>><%=mtm%>
				...
				<br>[(<%=((netPremiumPaidOrReceived) * lotSize * qty * -1)%> +
				<%=((tradeReversalExpense * -1))%>) ]</td>
		</tr>
	...
	<table align="center" width="1300" cellpadding="0" cellspacing="0"
		...
		<tr>
			...
			<td colspan="<%=(cols + 4)%>" align="left"><b>Options Chart
					Table </b></td>
		</tr>
 ...
 %><td><%=trade.buy ? "BUY" : "SELL"%> <%=trade.callOption ? "CALL" : "PUT"%>
					<%=trade.strikePrice%> @ <%=trade.boughtOrSoldPrice%></td> <%
				}
				%>
				<td>Net Amount</td>
			</b>
			<td align="left" width="450px;">Net Amount Explanation <br>
				[(netAmt * lotSize) - (netPremiumPaid * lotSize))] * qty
		</tr>
		...
		<tr>
			...
			newFinalAmtText = newFinalAmtText + " * " + lotSize + ")" + "-" + "("
					+ (netPremiumPaidOrReceived + " * " + lotSize + ")");
			%>
